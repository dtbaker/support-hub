...
class LinkedIn
{
     ...
     */
    public function api($resource, array $urlParams=array(), $method='GET', $postParams=array(), $return_header = false)
    {
        /*
        ...
        //$method that url
        $result = $this->getRequest()->send($url, $postParams, $method, $urlParams['format'], $return_header);

        if ($urlParams['format']=='json') {
            $new_result = json_decode($result, true);
	        if(is_array($new_result))return $new_result;
        }
