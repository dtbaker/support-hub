...
class shub_linkedin extends SupportHub_extension {
	...
	public function get_friendly_icon(){
		return '<img src="'.plugins_url('extensions/linkedin/linkedin-logo.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="shub_friendly_icon">';
	}
	...
	public function page_assets($from_master=false){
		...
		wp_register_style( 'support-hub-linkedin-css', plugins_url('extensions/linkedin/shub_linkedin.css',_DTBAKER_SUPPORT_HUB_CORE_FILE_), array(), '1.0.0' );
		wp_enqueue_style( 'support-hub-linkedin-css' );
		wp_register_script( 'support-hub-linkedin', plugins_url('extensions/linkedin/shub_linkedin.js',_DTBAKER_SUPPORT_HUB_CORE_FILE_), array( 'jquery' ), '1.0.0' );
	...
	public function get_message_details($shub_message_id){
		...
		foreach($messages as $message){
			...
			$data['shub_column_account'] .= '<div><img src="'.plugins_url('extensions/linkedin/linkedin-logo.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="linkedin_icon small"><a href="'.$linkedin_message->get_link().'" target="_blank">'.htmlspecialchars( $linkedin_message->get('linkedin_group') ? $linkedin_message->get('linkedin_group')->get( 'group_name' ) : 'Share' ) .'</a></div>';
			$data['shub_column_summary'] .= '<div><img src="'.plugins_url('extensions/linkedin/linkedin-logo.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="linkedin_icon small"><a href="'.$linkedin_message->get_link().'" target="_blank">'.htmlspecialchars( $linkedin_message->get_summary() ) .'</a></div>';
			...
			$data['shub_column_links'] .= '<div><img src="'.plugins_url('extensions/linkedin/linkedin-logo.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="linkedin_icon small">'. $link_clicks  .'</div>';
		}
		if(count($messages) && $link_clicks > 0){
			//$data['shub_column_links'] = '<div><img src="'.plugins_url('extensions/linkedin/linkedin-logo.png', _DTBAKER_SUPPORTHUB_CORE_FILE_).'" class="linkedin_icon small">'. $link_clicks  .'</div>';
		}
		return $data;
	...
	public function output_row($message, $settings){
		...
		<tr class="<?php echo isset($settings['row_class']) ? $settings['row_class'] : '';?> linkedin_message_row <?php echo !isset($message['read_time']) || !$message['read_time'] ? ' message_row_unread' : '';?>"
		    ...
		    <td class="shub_column_account">
			    <img src="<?php echo plugins_url('extensions/linkedin/linkedin-logo.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_);?>" class="linkedin_icon">
							    ...
							    foreach($send_groups as $linkedin_group_id => $tf){
								    ...
								    switch($linkedin_group_id){
									    case 'share':
										    ...
										    $linkedin_message->update('shub_status',_shub_MESSAGE_STATUS_PENDINGSEND);
		...
		switch($action){
			...
			case 'modal':
				if(isset($_REQUEST['sociallinkedinmessageid']) && (int)$_REQUEST['sociallinkedinmessageid'] > 0) {
					...
					if($shub_linkedin_message->get('shub_linkedin_message_id') == $_REQUEST['sociallinkedinmessageid']){
						...
						include( trailingslashit( $support_hub_wp->dir ) . 'extensions/linkedin/linkedin_message.php');
					}
			...
			case 'set-answered':
				...
				if(isset($_REQUEST['shub_linkedin_message_id']) && (int)$_REQUEST['shub_linkedin_message_id'] > 0){
					...
					if($shub_linkedin_message->get('shub_linkedin_message_id') == $_REQUEST['shub_linkedin_message_id']){
						$shub_linkedin_message->update('shub_status',_shub_MESSAGE_STATUS_ANSWERED);
						?>
			...
			case 'set-unanswered':
				...
				if(isset($_REQUEST['shub_linkedin_message_id']) && (int)$_REQUEST['shub_linkedin_message_id'] > 0){
					$shub_linkedin_message = new shub_linkedin_message(false, false, $_REQUEST['shub_linkedin_message_id']);
					if($shub_linkedin_message->get('shub_linkedin_message_id') == $_REQUEST['shub_linkedin_message_id']){
						$shub_linkedin_message->update('shub_status',_shub_MESSAGE_STATUS_UNANSWERED);
						?>
						jQuery('.sociallinkedin_message_action[data-id=<?php echo (int)$shub_linkedin_message->get('shub_linkedin_message_id'); ?>]').parents('tr').first().hide();
						<?php
					}
