...
class shub_envato_item extension SupportHub_item{
	...
	public function get_messages($search=array()){
		...
		//return get_m ultiple('shub_message',$search,'shub_message_id','exact','last_active');
	}
	...
	public function load_latest_item_data($debug = false){
		...
		if(isset($api_result['matches']) && is_array($api_result['matches'])){
			foreach($api_result['matches'] as $item_message){
				...
				$envato_message = new shub_message($this->envato_account, $this, false);
	...
	public function link_refresh(){
		return 'admin.php?page=support_hub_settings&tab=envato&manualrefresh&shub_account_id='.$this->get('shub_account_id').'&envato_item_id='.$this->get('item_id');
	}
