...
class shub_facebook extends SupportHub_extension {
	...
	public function get_friendly_icon(){
		return '<img src="'.plugins_url('extensions/facebook/facebook.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="shub_friendly_icon">';
	}
	...
	public function page_assets($from_master=false){
		...
		wp_register_style( 'support-hub-facebook-css', plugins_url('extensions/facebook/shub_facebook.css',_DTBAKER_SUPPORT_HUB_CORE_FILE_), array(), '1.0.0' );
		wp_enqueue_style( 'support-hub-facebook-css' );
		wp_register_script( 'support-hub-facebook', plugins_url('extensions/facebook/shub_facebook.js',_DTBAKER_SUPPORT_HUB_CORE_FILE_), array( 'jquery' ), '1.0.0' );
...
//		echo $sql;
//		echo "got : ".count($this->all_messages) ." messages. ";
	...
	public function get_message_details($shub_message_id){
		...
		foreach($messages as $message){
			...
			$data['shub_column_account'] .= '<div><img src="'.plugins_url('extensions/facebook/facebook.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="facebook_icon small"><a href="'.$facebook_message->get_link().'" target="_blank">'.htmlspecialchars( $page_or_group ? $page_or_group->get( 'page_name' ) : 'Feed' ) .'</a></div>';
			$data['shub_column_summary'] .= '<div><img src="'.plugins_url('extensions/facebook/facebook.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="facebook_icon small"><a href="'.$facebook_message->get_link().'" target="_blank">'.htmlspecialchars( $facebook_message->get_summary() ) .'</a></div>';
			...
			$data['shub_column_links'] .= '<div><img src="'.plugins_url('extensions/facebook/facebook.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="facebook_icon small">'. $link_clicks  .'</div>';
		}
		if(count($messages) && $link_clicks > 0){
			//$data['shub_column_links'] = '<div><img src="'.plugins_url('extensions/facebook/facebook.png', _DTBAKER_SUPPORTHUB_CORE_FILE_).'" class="facebook_icon small">'. $link_clicks  .'</div>';
		}
		return $data;
	...
	public function output_row($message, $settings = array()){
		...
		<tr class="<?php echo isset($settings['row_class']) ? $settings['row_class'] : '';?> facebook_message_row <?php echo !isset($message['read_time']) || !$message['read_time'] ? ' message_row_unread' : '';?>"
		    ...
		    <td class="shub_column_account">
			    <img src="<?php echo plugins_url('extensions/facebook/facebook.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_);?>" class="facebook_icon">
			...
			case 'modal':
				if(isset($_REQUEST['socialfacebookmessageid']) && (int)$_REQUEST['socialfacebookmessageid'] > 0) {
					...
					if($shub_facebook_message->get('shub_facebook_message_id') == $_REQUEST['socialfacebookmessageid']){
						...
						include( trailingslashit( $support_hub_wp->dir ) . 'extensions/facebook/facebook_message.php');
					}
