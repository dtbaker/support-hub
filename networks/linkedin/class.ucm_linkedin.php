...
class ucm_linkedin {
	...
	public function page_assets($from_master=false){
		...
		wp_register_style( 'support-hub-linkedin-css', plugins_url('networks/linkedin/social_linkedin.css',_DTBAKER_SUPPORT_HUB_CORE_FILE_), array(), '1.0.0' );
		wp_enqueue_style( 'support-hub-linkedin-css' );
		wp_register_script( 'support-hub-linkedin', plugins_url('networks/linkedin/social_linkedin.js',_DTBAKER_SUPPORT_HUB_CORE_FILE_), array( 'jquery' ), '1.0.0' );
	...
	public function load_all_messages($search=array(),$order=array()){
		$sql = "SELECT m.*, m.last_active AS `message_time`, mr.read_time FROM `"._support_hub_DB_PREFIX."social_linkedin_message` m ";
		$sql .= " LEFT JOIN `"._support_hub_DB_PREFIX."social_linkedin_message_read` mr ON m.social_linkedin_message_id = mr.social_linkedin_message_id";
		$sql .= " WHERE 1 ";
	...
	public function get_message_details($social_message_id){
		...
		foreach($messages as $message){
			...
			$data['social_column_social'] .= '<div><img src="'.plugins_url('networks/linkedin/linkedin-logo.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="linkedin_icon small"><a href="'.$linkedin_message->get_link().'" target="_blank">'.htmlspecialchars( $linkedin_message->get('linkedin_group') ? $linkedin_message->get('linkedin_group')->get( 'group_name' ) : 'Share' ) .'</a></div>';
			$data['social_column_summary'] .= '<div><img src="'.plugins_url('networks/linkedin/linkedin-logo.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="linkedin_icon small"><a href="'.$linkedin_message->get_link().'" target="_blank">'.htmlspecialchars( $linkedin_message->get_summary() ) .'</a></div>';
			// how many link clicks does this one have?
			$sql = "SELECT count(*) AS `link_clicks` FROM ";
			$sql .= " `"._support_hub_DB_PREFIX."social_linkedin_message` m ";
			$sql .= " LEFT JOIN `"._support_hub_DB_PREFIX."social_linkedin_message_link` ml USING (social_linkedin_message_id) ";
			$sql .= " LEFT JOIN `"._support_hub_DB_PREFIX."social_linkedin_message_link_click` lc USING (social_linkedin_message_link_id) ";
			$sql .= " WHERE 1 ";
			...
			$data['social_column_links'] .= '<div><img src="'.plugins_url('networks/linkedin/linkedin-logo.png', _DTBAKER_SUPPORT_HUB_CORE_FILE_).'" class="linkedin_icon small">'. $link_clicks  .'</div>';
		}
		if(count($messages) && $link_clicks > 0){
			//$data['social_column_links'] = '<div><img src="'.plugins_url('networks/linkedin/linkedin-logo.png', _DTBAKER_SUPPORTHUB_CORE_FILE_).'" class="linkedin_icon small">'. $link_clicks  .'</div>';
		}
		return $data;
	...
	public function get_unread_count($search=array()){
		...
		$sql = "SELECT count(*) AS `unread` FROM `"._support_hub_DB_PREFIX."social_linkedin_message` m ";
		$sql .= " WHERE 1 ";
		$sql .= " AND m.social_linkedin_message_id NOT IN (SELECT mr.social_linkedin_message_id FROM `"._support_hub_DB_PREFIX."social_linkedin_message_read` mr WHERE mr.user_id = '".(int)get_current_user_id()."' AND mr.social_linkedin_message_id = m.social_linkedin_message_id)";
